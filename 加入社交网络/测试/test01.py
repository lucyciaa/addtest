...
if __name__ == '__main__':
    ...
    for user in range(start, end):
        ...
        for i in range(1, n + 1):
            ...
            for j in range(1, m + 1):
                if (G_train.has_edge(i, str(j))):
                    goodsToUser += f_goods[j - 1] / G_train.degree(str(j))
    ...
    #     # 首先向商品传播资源
        ...
        for i in range(1, m + 1):
            ...
            for j in range(1, n + 1):
                if (G_train.has_edge(j, str(i))):
                    userToGoods += p * f_user[j - 1] * int(G_train.has_edge(j, str(i))) / G_train.degree(j)
    ...
    #     # 第四步 用户再次向商品传播资源
        for i in range(1, m + 1):
            ...
            for j in range(1, n + 1):
                if (G_train.has_edge(j, str(i))):
                    userToGoods += f_user_to_user[j - 1] * int(G_train.has_edge(j, str(i))) / G_train.degree(j)
