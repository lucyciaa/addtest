...
if __name__ == '__main__':
    ...
    a_train = a_train.reshape(531859, 3)
    ...
    a_test = a_test.reshape(132965, 3)
    ...
    user_list = user_list.reshape(531859, 3)
    ...
    with open('data/trust_data.txt','r') as f:
        f_u = f.readlines()
    for line in f_u:
        if(len(user_goods_dict[int(line.split(' ')[1])]) > 0):
    ...
    for user in range(start, end):
        ...
        print('f_user', f_user, f_user.sum())
        ...
        print('f_final',f_final, f_final.sum())
        ...
        print('f_user_to_user', f_user_to_user, f_user_to_user.sum())
        # 第四步 用户再次向商品传播资源
        for i in G_train.nodes():
            userToGoods = 0
            if (G_train.nodes[i]['type'] == 'item'):
